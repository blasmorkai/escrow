...
const CONTRACT_VERSION: &str = env!("CARGO_PKG_VERSION");
    ...
    // #1 WE ARE NOT CHECKING IF WE HAVE GOT ENOUGH FUNDS IN THE CONTRACT TO BankMessage::Send{} when the arbiter specifies quantity.
    //      If the send fails (e.g., user specified more coins than were in the escrow), 
    //      all state changes in this contract would be reverted... just as if we returned an Unauthorized error.
    // #2 WHEN DOES THE CONTRACT RECEIVE FUNDS? Async BankMsg::Send{ }? 
    // #3 IF EXPIRATION IS NOT SET UP ON INSTANTIATION, FUNDS WILL NOT BE RETRIEVABLE AFTER IT HAS EXPIRED AND WILL STAY IN THE CONTRACT
    // 	    Arbitrer can retrieve funds will it has not expired in this case.
...
pub fn instantiate(
    ...
    // The expiration set on instantiation can not be in the past.
...
// arbiter sends coins (quantity or all) to recipient if contract is not expired
fn execute_approve(
    deps: DepsMut,
    env: Env,
    info: MessageInfo,
    ...
    // #1 WE ARE NOT CHECKING IF WE HAVE GOT ENOUGH FUNDS TO SEND when the arbiter specifies quantity.
    // #2 WHEN DOES THE CONTRACT RECEIVE FUNDS? Async BankMsg::Send{ }? 
...
mod tests {
    ...
    fn execute_approve() {
        ...
        // .expect("error_msg if None")   - Returns the contained Some value, consuming the self value.
    ...
    fn handle_refund_no_expiration() {
        ...
        // cannot release because the expiration was not set up
        let msg = ExecuteMsg::Refund {};
        let mut env = mock_env();
